...
class NLMS_predictor:        
    ...
    def predict(self,past,idx,j):
        if idx == 0:
            ...
            self.filt.adapt(past[j][idx-1],np.append(np.reshape(past[:,idx-self.n-1:idx-1],(-1)),np.reshape(past[:j,idx-1],(-1))))
            return self.filt.predict(np.append(np.reshape(past[:,idx-self.n:idx],(-1)),np.reshape(past[:j,idx],(-1))))
#            self.filt.adapt(past[j][idx-1]-past[j][idx-2],np.reshape(past[:,idx-self.n-1:idx-1],(-1)))
#            return self.filt.predict(np.reshape(past[:,idx-self.n:idx],(-1)))+past[j][idx-1]
